{<template>
	<view class = 'applyAccident'>
		<view class = 'applyAccidentItem' v-for = 'item in childList' @click="handleAccident(item)">
			<img class="applyAccidentItemImg" :src="`/static/icon/${item.icon}.png`" alt="">
			{{item.title}}
		</view>
	</view>
</template>
...
<style lang="scss">
	.applyAccident{
		...
		.applyAccidentItem{
			height: 168rpx;
			line-height: 168rpx;
			background: linear-gradient(to right, #6CA3FE, #4487F4);
			color: #fff;
			border-radius: 16rpx;
			margin-bottom: 36rpx;
			padding-left: 48rpx;
